...
The two TS-plugin adapters (opencode, openclaw) that used to live here as standalone files have been promoted to first-class providers in 1.3 — `llmoji install opencode` / `llmoji install openclaw` now renders and writes the plugin files for you. The canonical TypeScript port of the kaomoji validator lives at [`llmoji/_plugins/_kaomoji_taxonomy.ts.partial`](../llmoji/_plugins/_kaomoji_taxonomy.ts.partial) and is spliced into both rendered plugins at install time.
